	...
	**/
	...
	public:
		...
		**/
		...
		MyLibException(const Logger::ELoggingLevel _level, const std::source_location _sourceLocation, const std::format_string<Args...> _expComment, Args&&... _expCommentFormat)
			:m_ExpLevel(_level), m_ExpLoc(_sourceLocation) {
			m_what = Logger::Logging(_level, _sourceLocation, _expComment, std::forward<Args>(_expCommentFormat)...);
		}
	...
	protected:
		const Logger::ELoggingLevel m_ExpLevel;
